...
//    @Select("select * from `user` where `is_deleted` = 0 limit #{pageNum},#{pageSize}")
    @Select("select user_details.id userId, nickname username,email,head,(select count(*) from item where seller_id=user_details.id) releasedNum,(select count(*) from item where seller_id=user_details.id and is_audit=1) unpassedNum,user_details.is_deleted is_deleted from user_details order by ${sort} ${order} limit #{pageNum},#{pageSize}")
    List<AuditUserVO> getAllUser(int pageNum, int pageSize, String sort, String order);

    @Select("select * from `user_details` limit #{pageNum},#{pageSize}")
    List<UserDetails> getUsers(Integer pageNum,Integer pageSize);
