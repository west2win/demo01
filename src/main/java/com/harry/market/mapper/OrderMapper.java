...
public interface OrderMapper extends BaseMapper<Order> {
    ...
    @Select("select `order`.id orderId,(select nickname from user_details where user.id=`user_order`.buyer_id limit 1) buyerName, (select head from user_details where user_details.id=`user_order`.buyer_id) buyerHead, (select nickname from user_details where user.id=`order`.seller_id limit 1) sellerName, (select head from user_details where user_details.id=`order`.seller_id) sellerHead, (select `name` from item where item.id=`order`.item_id) itemName, (select photo from item where item.id=`order`.item_id) itemPhoto, per_price price, number from `user_order` inner join `order` on `order`.id=`user_order`.id inner join `user` on user_order.buyer_id = user.id where buyer_id = #{userId}")
