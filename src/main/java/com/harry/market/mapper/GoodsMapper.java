...
//    @Select("select  id,name,introduction,photo,price from item limit #{pageNum},#{pageSize}")
    ...
    @Select("select i.is_audit,i.id,i.kind,name,i.seller_id sellerId,ud.nickname sellerName,ud.head sellerHead,i.introduction,photo,price,i.gmt_modified,number from item i inner join user_details ud on i.seller_id=ud.id where i.id = #{id}")
    ...
    @Select("select  i.id,i.kind,name,ud.id sellerId,ud.nickname sellerName,i.introduction,photo,price,i.gmt_modified,i.number from item i inner join user_details ud on i.seller_id=ud.id where i.id=#{itemId}")
    ItemVO selectItembyId(Long itemId);

    @Select("select  i.is_audit,i.id,i.kind,name,ud.id sellerId,ud.nickname sellerName,i.introduction,photo,price,i.gmt_modified from item i inner join user_details ud on i.seller_id=ud.id where is_audit=2 order by price ${order} limit #{pageNum},#{pageSize}")
